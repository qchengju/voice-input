...
function stopAudioRecording() {
    if (mediaRecorder && isRecording) {
        try {
            ...
            console.warn('stop media recorder error', err);
        }
    }
    if (mediaStream) {
    ...
    statusTextSpan.innerText = '已停止录音，结果已生成。';
}

function startAudioRecording() {
    if (!navigator.mediaDevices || !window.MediaRecorder) {
        statusTextSpan.innerText = '❌ 当前浏览器不支持录音功能，请使用 Chrome/Edge。';
        return;
    }

    const recognitionStarted = initAndStartRecognition();
    if (!recognitionStarted) {
        statusTextSpan.innerText = '❌ 语音识别启动失败，无法同时录音转文字。';
        return;
    }

    pendingRecordingStart = true;
    statusTextSpan.innerText = '🎙️ 正在准备录音和识别，请允许麦克风访问...';
}

function beginRecordingStream() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            ...
            try {
                ...
                statusTextSpan.innerText = '🎙️ 正在录音，正在转换文字...';
            } catch (err) {
                console.error('录音启动失败', err);
                statusTextSpan.innerText = '❌ 录音启动失败，请重试。';
                stopRecognition();
            }
        })
        .catch(err => {
            console.error('录音权限获取失败', err);
            statusTextSpan.innerText = '❌ 无法获取麦克风权限，录音失败。';
            stopRecognition();
        });
}
...
downloadAudioBtn.addEventListener('click', () => {
    ...
    anchor.href = url;
    anchor.download = 'recorded-voice.webm';
